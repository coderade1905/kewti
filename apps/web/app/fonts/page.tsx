...
export default function BrowseFonts(): JSX.Element {
  ...
  useEffect(() => {
    async function fetchFontRegistry() {
      try {
        ...
        // Check URL search parameters to auto-open shared font
  ...
  const handleShareFont = useCallback((font: FontItem, e?: React.MouseEvent) => {
    ...
    const shareUrl = `${window.location.origin}${window.location.pathname}?font=${font.name}`
    navigator.clipboard.writeText(shareUrl)
    showToast(`Copied link for "${font.title}" to clipboard!`)
  }, [])
...
@font-face {
  ...
  // Generate CSS import snippet for drawer
  const getCssImportSnippet = () => {
    ...
    const fontFaceRules = fontFiles
      .map((file) => {
        ...
        return `@font-face {
  font-family: '${selectedFont.name}_${variant}';
  src: url('/path/to/src/kewti/${file.path}') format('${selectedFont.fontType}');
  font-display: swap;
}`
      })
      .join("\n\n")
                          ...
                          <div className="mb-4 flex items-start justify-between border-b border-neutral-800/60 pb-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="rounded bg-neutral-800 px-1.5 py-0.5 font-mono text-[10px] text-neutral-400">
                                {fontFiles.length}{" "}
                                {fontFiles.length === 1 ? "variant" : "variants"}
                              </span>
                            </div>
                          </div>
                ...
                <div>
                  ...
                  {selectedFont?.createdBy && (
                    <p className="mt-0.5 text-xs text-neutral-300">
                      Created by: {selectedFont.createdBy}
                    </p>
                  )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedFont && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-md border border-neutral-800 bg-neutral-950 px-2.5 py-1.5 text-xs font-medium text-neutral-300 transition-colors hover:bg-neutral-800 hover:text-white"
                    >
                      <svg
